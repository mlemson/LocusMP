...
class LocusLobbyUI {
	...
	_initServerBrowseToggle() {
		...
		toggleBtn.addEventListener('click', async () => {
			...
			// Als Socket.IO niet beschikbaar is, probeer on-demand te laden
			...
			// Socket.IO is al beschikbaar — normaal gedrag
			list.style.display = '';
			toggleBtn.textContent = '🌐 Lobbys verbergen';
			this._refreshServerBrowser();
		});
	...
	_loadSocketIO(baseUrl) {
		...
		return new Promise((resolve, reject) => {
			...
			s.onload = resolve;
			s.onerror = () => reject(new Error('Socket.IO script kon niet geladen worden'));
	...
	_renderDeckOverview() {
		...
		if (obj) {
			...
			const dObjTarget = isClassicDeck ? '<span class="mp-obj-icon-target"></span>' : '🎯';
			const dObjFail = isClassicDeck ? '<span class="mp-obj-icon-fail"></span>' : '❌';
			const dTrophy = isClassicDeck ? '' : '🏆 ';

			objectiveHtml = `
				<div class="mp-deck-objective-panel">
					<h4 class="mp-deck-section-title">${dObjTarget} Jouw Doelstelling</h4>
					<div class="mp-deck-objective-card ${achieved ? 'achieved' : ''} ${failed ? 'failed' : ''}">
						<div class="mp-deck-obj-name">${achieved ? dObjOk : (failed ? dObjFail : dObjTarget)} ${this._escapeHtml(obj.name)}</div>
	...
	_renderMyObjective() {
		...
		const objIconTarget = isClassicObj ? '<span class="mp-obj-icon-target"></span>' : '🎯';
		const objIconFail = isClassicObj ? '<span class="mp-obj-icon-fail"></span>' : '❌';
		const trophyIcon = isClassicObj ? '' : '🏆 ';

		container.innerHTML = `
			<div class="mp-objective-badge ${achieved ? 'objective-achieved' : ''} ${failed ? 'objective-failed' : ''}">
				<div class="mp-objective-title">${achieved ? objIconOk : (failed ? objIconFail : objIconTarget)} ${this._escapeHtml(obj.name)} ${pts ? `<span style="color: var(--mp-gold); font-size: 0.8rem;">(${pts}pt)</span>` : ''}</div>
	...
	_openPerkPopup(onClose = null) {
		const Rules = window.LocusGameRules;
		if (!Rules || !Rules.PERK_BRANCHES) {
			if (onClose) onClose();
			return;
		}
		...
		// Close handlers
		overlay.querySelector('#mp-perk-popup-close').addEventListener('click', closePopup);
		if (!isChoosingGoals) {
			overlay.addEventListener('click', (e) => {
		...
		overlay.querySelectorAll('.mp-perk-unlock-btn:not(.disabled)').forEach(btn => {
			btn.addEventListener('click', () => this._handleChoosePerk(btn.dataset.perkId, btn, onClose));
		});
	}

	async _handleChoosePerk(perkId, sourceEl = null, onClose = null) {
		try {
